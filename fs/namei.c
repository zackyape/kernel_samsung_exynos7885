...
static inline int may_create(struct vfsmount *mnt, struct inode *dir, struct dentry *child)
{
	...
	if (child->d_inode &&
		...
		/*
		 * Untuk user-app non-root:
		 * - kalau inode ditandai SUS_PATH, jangan izinkan create/overwrite.
		 * - kalau parent tidak punya izin tulis, balikin error permission normal.
		 * - kalau punya izin, pura-pura file tidak ada: -ENOENT.
		 */
		error = inode_permission2(mnt, dir, MAY_WRITE | MAY_EXEC);
		if (error)
			return error;
